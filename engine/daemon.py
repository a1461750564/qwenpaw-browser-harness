...
PROFILES = [
    ...
    Path.home() / ".qwenpaw-chrome/sunmai",  # QwenPaw persistent profile
]
...
def get_ws_url():
    if url := os.environ.get("BU_CDP_WS"):
        # Only use BU_CDP_WS if it's not a stale localhost reference.
        # If the user is running a local browser on a dynamic port (e.g. 9223),
        # a hardcoded localhost URL in .env will cause a connection failure.
        # In that case, we fall back to scanning PROFILES for DevToolsActivePort.
        if "localhost" in url or "127.0.0.1" in url:
            import socket
            try:
                # Try connecting to the port in the URL.
                # Extract port from ws://host:port/path
                port = url.split(":")[2].split("/")[0]
                probe = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                probe.settimeout(1)
                probe.connect(("127.0.0.1", int(port)))
                probe.close()
                return url # It's alive!
            except Exception:
                log(f"BU_CDP_WS ({url}) appears stale, falling back to local discovery")
                # Fall through to profile scan
        else:
            return url # Remote URL, use it.
    ...
    # Fallback 2: Scan common CDP ports via HTTP JSON endpoint if DevToolsActivePort is missing
    # This handles cases like custom --user-data-dir where the file might not be created.
    for port in range(9220, 9230):
        try:
            with urllib.request.urlopen(f"http://127.0.0.1:{port}/json/version", timeout=2) as resp:
                data = json.loads(resp.read())
                ws_url = data.get("webSocketDebuggerUrl")
                if ws_url and ("localhost" in ws_url or "127.0.0.1" in ws_url):
                    log(f"Found active CDP on port {port} via HTTP JSON")
                    return ws_url
        except Exception:
            continue

    # Fallback: Sniff standard debugging ports (9220-9229) if DevToolsActivePort is missing.
    # This handles cases where Chrome is running but the file is missing/permissions denied.
    for port in range(9220, 9230):
        try:
            ...
            if s.connect_ex(("127.0.0.1", port)) == 0:
                # Port open, verify via CDP HTTP endpoint
                try:
                    ...
                    s.close()
                    if ws_url:
                        log(f"found live Chrome on port {port} via /json/version scan")
                        return ws_url
                except Exception:
                    pass
            s.close()
    ...
    raise RuntimeError(f"DevToolsActivePort not found in {[str(p) for p in PROFILES]} and no live browser found on ports 9220-9229 — enable chrome://inspect/#remote-debugging, or set BU_CDP_WS for a remote browser")
